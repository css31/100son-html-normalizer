...
> Moteur de **normalisation HTML configurable** pour WordPress — 8 préréglages prêts à l'emploi, garde-fou anti-perte de contenu, et API publique consommable par d'autres plugins.

[![Version](https://img.shields.io/badge/version-0.1.0-orange.svg)](CHANGELOG.md)
[![PHP](https://img.shields.io/badge/PHP-8.3+-blue.svg)](#stack-technique)
[![WordPress](https://img.shields.io/badge/WordPress-6.8+-21759b.svg)](#stack-technique)
...
paragraphes vides, headings sans contenu, balises `<br>` à la chaîne, styles inline laissés
par d'anciens éditeurs visuels, shortcodes orphelins de plugins désinstallés, listes en `*`
...
**brique amont** d'une chaîne de migration plus large (notamment du plugin frère
`100son-so-to-blocks`, dédié à la conversion SiteOrigin → Gutenberg), mais il est
**utilisable seul** : un cleaner HTML générique, configurable, testable, auditable.
...
| **P6** | `RemoveInlineStyles` | Supprime tout `style="…"` inline (option `keep_text_align` pour préserver les alignements) |
...
| **Tester un fragment** | Coller un bout de HTML, voir le résultat normalisé en source + rendu. Utile pour debug et pour comprendre l'effet des préréglages. |
...
### API publique — utilisation depuis un autre plugin

Le plugin expose **un seul filtre** WordPress :
...
**Contrat de robustesse** : si le plugin est désactivé, le filtre n'existe pas et `apply_filters` renvoie `$html` inchangé. Le code consommateur n'a rien à protéger.

Exemple d'usage depuis un autre plugin :
...
- **Composer** pour l'installation des dépendances (toolchain de dev uniquement)
...
Puis activer le plugin depuis l'admin WordPress (**Extensions → 100son HTML Normalizer**).

### Pourquoi `composer install` ?

Le dossier `vendor/` n'est pas versionné (convention PHP/Composer). Il est régénéré à
l'identique à partir de `composer.lock` qui, lui, est commité — garantissant les **versions
exactes** des dépendances chez tout le monde.
...
```
100son-html-normalizer/
├── 100son-html-normalizer.php      Header WP + bootstrap + garde-fous PHP/WP
...
│   │   ├── Dom/DomHtml.php         Wrapper DOMDocument (parsing/sérialisation)
...
Tester le filtre depuis WP-CLI (sandbox DevKinsta) :
...
HTML Normalizer est **optionnel** côté SO to Blocks : sans lui, le plugin a un chemin dégradé
...
Repo : [github.com/css31/100son-html-normalizer](https://github.com/css31/100son-html-normalizer)
